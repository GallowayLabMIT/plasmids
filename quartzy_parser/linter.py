"""Various lint checks to ensure plasmid metadata stays up to date."""
...
def lint_pKG_number(plasmid: Plasmid) -> Optional[str]:
    """Check if the item-name pKG number matches the pKG metadata entry."""
    ...
    if match is not None:
        ...
        if item_name_pKG != plasmid.pKG:
            return (
                f"Item {plasmid.name} has inconsistent pKG numbering."
...
def lint_addgene_alt_names(plasmid: Plasmid) -> Optional[str]:
    """Check to see if the Addgene item name is properly set."""
    if plasmid.vendor is None or plasmid.vendor != "Addgene":
        return None
    if re.match(r"^\d+$", plasmid.alt_name) is None:
        return (
            "Plasmid has the Vendor field set to Addgene, but has a suspicious catalog "
            + f"number: {plasmid.alt_name}. The catalog number should just be the Addgene number!"
        )
    return None


def lint_addgene_map(plasmid: Plasmid) -> Optional[str]:
    """Check to see if Addgene plasmids have attached map."""
    if plasmid.vendor is None or plasmid.vendor != "Addgene":
        return None
    if len(plasmid.attachments) == 0:
        return (
            "Addgene plasmid is missing a plasmid map! "
            + "Please add a sequence downloaded from the entry on Addgene."
        )
    return None


def lint_plasmid_name(plasmid: Plasmid) -> Optional[str]:
    """Check that the plasmid name is non-empty."""
    if len(plasmid.name) == 0:
        return "Plasmid has an empty plasmid name field!"
    return None
...
def lint_attachments(plasmid: Plasmid) -> Optional[str]:
    ...
    if len(plasmid.attachments) > 0 or "no_map" in plasmid.technical_details:
        return None
    return (
        "Plasmid is missing a plasmid-map attachment! If this is intended, add `no_map` to Technical Details"
    )


# Lint antibiotic resistances
def lint_antibiotic(plasmid: Plasmid) -> Optional[str]:
    """Check to see that we aren't specifying combination antibiotics or short names."""
    invalid_resistances = [
        "AMP",
        "Kanamycin/chlor",
    ...
    for resistance in plasmid.resistances:
        if resistance in invalid_resistances:
            return (
                "Plasmid has an invalid (shortened or dual-resistance) antibiotic resistance "
                + f"entry: {resistance}"
            )
...
def lint_antibiotics_match_type(plasmid: Plasmid) -> Optional[str]:
    """
    Check that the antibiotic matches the expected value for the given plasmid type.
...
def lint_plasmids(plasmids: List[Plasmid]) -> None:
    """
    Check plasmids for consistency.

    Updates the plasmid list in place with lint results.
    """
