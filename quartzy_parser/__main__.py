"""Script to run plasmid error/warning calculations."""
...
def get_flags(users: List[models.User], plasmids: List[models.Plasmid]):
    """Compute plasmid errors and warnings."""
    ...
    for user in users:
        ...
        if user.full_name == "Galloway Lab":
            default_user = user
    ...
    for plasmid in plasmids:
        if plasmid.owner_id in list(users_by_id.keys()):
            ...
            user = default_user.full_name

        if plasmid.warnings:
            if user not in list(warnings.keys()):
                warnings[user] = [plasmid]
            else:
                warnings[user].append(plasmid)
        if plasmid.errors:
            if user not in list(errors.keys()):
                errors[user] = [plasmid]
            else:
                errors[user].append(plasmid)
...
if Path("credentials.json").is_file():
    ...
    arg_parser.exit(
        1,
        (
            "Cannot find credentials! Create a `credentials.json` file that looks "
